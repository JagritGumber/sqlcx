// mysql-connector-python driver generator for Python
...
fn escape_sql_mysql(s: &str) -> (String, Vec<u32>) {
...
fn generate_query_function(query: &QueryDef) -> String {
    ...
    let (escaped_sql, param_indices) = escape_sql_mysql(&query.sql);
...
mod tests {
    ...
    fn generates_client_file() {
        let gen = MysqlConnectorGenerator;
