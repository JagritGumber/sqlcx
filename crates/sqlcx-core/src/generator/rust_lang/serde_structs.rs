...
fn collect_imports(
    ...
    for table in tables {
        for col in &table.columns {
            check_type_imports(&col.sql_type, overrides, &mut needs_chrono, &mut needs_serde_json);
        }
    }
...
fn check_type_imports(
    sql_type: &SqlType,
    overrides: &Overrides,
    needs_chrono: &mut bool,
    needs_serde_json: &mut bool,
) {
    if let Some(elem) = &sql_type.element_type {
        check_type_imports(elem, overrides, needs_chrono, needs_serde_json);
        return;
    }
    match &sql_type.category {
...
fn generate_select_struct(
    ...
    let fields: Vec<String> = table
        .columns
        .iter()
        .map(|col| format!("    pub {}: {},", col.name, select_field_type(col, overrides)))
        .collect();
...
fn generate_insert_struct(
    ...
    let fields: Vec<String> = table
        .columns
        .iter()
        .map(|col| format!("    pub {}: {},", col.name, insert_field_type(col, overrides)))
        .collect();

    format!(
        "/// Insert type for {} \u{2014} columns with defaults are optional\n#[derive(Debug, Clone, Serialize, Deserialize)]\npub struct {} {{\n{}\n}}",
        table.name,
        name,
        fields.join("\n")
    )
...
impl SerdeStructGenerator {
    pub fn generate_schema_file(&self, ir: &SqlcxIR, overrides: &Overrides) -> String {
        ...
        // Imports
        let imports = collect_imports(&ir.tables, overrides);
...
mod tests {
    ...
    fn date_type_mapping() {
        ...
        assert_eq!(date_type("TIMESTAMP WITHOUT TIME ZONE"), "chrono::NaiveDateTime");
