...
// Bun.sql uses the default TS type mapping (same as node-postgres).
...
fn generate_query_function(query: &QueryDef) -> String {
    ...
    let row_type = generate_row_type(&BunSqlTypeMap, query);
    let params_interface = generate_params_type(&BunSqlTypeMap, query);
