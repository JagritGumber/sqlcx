...
use crate::generator::{GeneratedFile, LanguagePlugin, SchemaGenerator};
...
impl PythonPlugin {
    pub fn new(schema: &str, _driver: &str) -> Result<Self> {
        Ok(Self {
...
impl LanguagePlugin for PythonPlugin {
    fn generate(&self, ir: &SqlcxIR, config: &TargetConfig) -> Result<Vec<GeneratedFile>> {
        ...
        // Schema-only for now (no driver generator yet)
        let files = vec![schema_gen.generate(ir, overrides)?];
        Ok(files)
    }
}
