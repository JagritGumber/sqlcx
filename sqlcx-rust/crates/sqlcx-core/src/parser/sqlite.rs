...
fn parse_column_line(line: &str) -> Option<ParsedColumn> {
    ...
    let constraint_re = Regex::new(
        r"(?i)^(PRIMARY\s+KEY|CONSTRAINT|UNIQUE|CHECK|FOREIGN\s+KEY)",
    )
    .unwrap();
    ...
    let name_re = Regex::new(r#"^(?:`(\w+)`|"(\w+)"|\[(\w+)\]|(\w+))\s+"#).unwrap();
    ...
    let (raw_type, rest) = if let Some(cap) = type_re.captures(after_name) {
    ...
    let auto_inc_re = Regex::new(r"(?i)\bAUTOINCREMENT\b").unwrap();
